    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into AddressBook values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getAddres()+"')";
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',address='"+person.getAddres()+"'where name= '"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.setAddres(rs.getString("address"));
                         conn.close();
        }
        }
         catch (SQLException ex) {
